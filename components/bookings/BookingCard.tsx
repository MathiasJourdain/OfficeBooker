  ...
  return (
    <Card className="overflow-hidden flex flex-col sm:flex-row shadow-sm hover:shadow-md transition">
      <div className="sm:w-48 h-32 sm:h-auto bg-gray-200 relative">
        {booking.rooms?.image_url ? <img src={booking.rooms.image_url} alt="Salle" className="absolute inset-0 w-full h-full object-cover" /> : <div className="flex items-center justify-center h-full text-gray-400">{t("noImage")}</div>}
      </div>
      <div className="flex-1 p-6 flex flex-col justify-between">
        <div>
          <h3 className="font-bold text-xl text-gray-800">{booking.rooms?.name || t("unknownRoom")}</h3>
        ...
        <div className="mt-4 flex justify-end">
          <form action={cancelBooking}>
            ...
            <Button variant="destructive" size="sm" className="flex items-center gap-2"><Trash2 className="w-4 h-4" />{t("cancel")}</Button>
          </form>
        </div>
      </div>
    </Card>
